...
export default function Login() {
  ...
  const handleVerify2FA = async () => {
    ...
    try {
      ...
      if (data.user.isSuperAdmin) navigate('/super-admin', { replace: true });
      else navigate('/dashboard', { replace: true });
    } catch (err: any) {
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 w-full">
          ...
          <div className="space-y-6">
            <h1 className="font-display text-4xl xl:text-5xl font-bold text-white leading-tight">
              {isCorporateLogin ? 'Corporate Control Center' : 'Simplify your financial management'}
            </h1>
            <p className="text-white/80 text-lg max-w-md">
              {isCorporateLogin
                ? 'Sign in to the corporate portal to manage organization-wide settings, operations, and content.'
          ...
          <Card className="w-full max-w-md border-0 shadow-none lg:shadow-card lg:border">
            <CardHeader className="text-center pb-2">
              ...
              <CardTitle className="font-display text-2xl">
                {isCorporateLogin ? 'Corporate User Login' : 'Welcome back'}
              </CardTitle>
            <CardDescription>
                {isCorporateLogin ? 'Authorized corporate administrators only' : 'Sign in to your VAYPR account'}
              </CardDescription>
            </CardHeader>
